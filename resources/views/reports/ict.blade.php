...
<head>
    ...
    <style>
        ...
        /* Unified pagination footer — text on the left, buttons on the right,
           same line on desktop, stacks on small screens. Same look used on the
           customer and product reports too. */
    ...
    <div class="container-fluid mt-5 mb-5 px-4">
        <div class="modern-card">
            ...
            <div class="card-body p-0">
                ...
                @if($ictData instanceof \Illuminate\Pagination\LengthAwarePaginator && $ictData->total() > 0)
                <div class="report-pagination-wrap">
                    <div class="results-info">
                        Showing {{ $ictData->firstItem() }} to {{ $ictData->lastItem() }} of {{ $ictData->total() }} results
                    </div>
                    {{ $ictData->links('vendor.pagination.buttons-only') }}
                </div>
                @endif
            </div>
        </div>
