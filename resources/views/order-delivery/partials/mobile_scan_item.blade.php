...
$eanMissing = !empty($scan->sku) && !empty($scan->ean_code) && strval($scan->ean_code) == strval($scan->sku);
@endphp
<div class="history-item-minimal"
    id="mobile-scan-{{ $scan->id }}"
    data-scan-status="{{ isset($scan->is_virtual) ? 'missing' : 'scanned' }}"
    data-ean-missing="{{ $eanMissing ? '1' : '0' }}"
    ...
    <div class="item-data" style="width: 100%;">
        ...
        <div class="text-center mb-2">
            ...
            @if($eanMissing)
            <div class="ean-missing-wrapper" style="display: flex; justify-content: center; margin-top: 4px;">
        ...
        <div class="metrics-grid mb-3">
            <div class="metric-box">
                ...
                <span class="metric-val row-ordered-val" data-ean="{{ $scan->ean_code }}">{{ $ordered }}</span>
            </div>
            <div class="metric-box">
                <span class="metric-label">Scanned</span>
                <span class="metric-val row-scanned-val" data-ean="{{ $scan->ean_code }}">{{ $scannedSoFar }}</span>
            </div>
            <div class="metric-box">
                <span class="metric-label">Rest</span>
                <span class="metric-val remaining row-remaining-val" data-ean="{{ $scan->ean_code }}">{{ $remaining }}</span>
            </div>
            <div class="metric-box">
