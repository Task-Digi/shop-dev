...
class DashboardMultipleController extends Controller
{
    /**
     * @var
     */
    ...
    public $access;
    public $customerName = [
        'høyskolen-kristiania' => 'HØYSKOLEN KRISTIANIA',
    ];

    function __construct(Request $request)
    {
        if(isset($request->user) && $request->user == 'høyskolen-kristiania')
            $this->user = array(500106, 500107, 500108, 500109, 500110);
    ...
    public function dashboard(Request $request)
    {
        ...
        $companies = $this->customerName;
        if (!empty($user) && $standard['id'] !== null) {
            ...
            $data['noOfResponse']['week'] = Respond::select('COUNT(Respond_ID)')
                ...
                ->whereIn('Respond_Customer_ID', $user)
            ...
            $data['sourceCount'] = Response::select(DB::raw('Respons_Source_ID,  COUNT(Respons_Source_ID) as Source_Count'))
                ...
                ->whereIn('Respons_Customer_ID', $user)
